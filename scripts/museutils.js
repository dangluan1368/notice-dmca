    ...
    for (var f = [], j = $(window), h = function(a) {
        ...
        if (h(k)) {
            if (k = Muse.Utils.allStyleSheetRulesFromOneSheet(k, b), null != k)
                for (var l = 0; l < k.length; l++) f.push(k[l])
        } else if (Muse.Utils.isDefined(k.selectorText))
            if (c) b(k.selectorText) && f.push(k);
            else
                for (var l = k.selectorText.split(/\s*,\s*/), m = 0; m < l.length; m++) b(l[m]) && f.push(k)
    }
    return f.length ?
        f : null
};
    ...
    if (!d || d == "_self") {
        ...
        if (f && d > g) {
            var a = c.data("scrollWrapper"),
                ...
                i = Math.min(f, d.top + (a && !a.isStandard() ? a.scrollTop() :
                    0)),
